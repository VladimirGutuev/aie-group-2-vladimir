...
- **Ссылка на репозиторий:** _добавить перед сдачей_

Проект решает задачу распознавания автомобильных номеров на изображениях с парковочных камер.
Решение строится как двухступенчатый пайплайн: детекция области номера (YOLOv8) и OCR (EasyOCR).
Цель — REST API, принимающий изображение и возвращающий строку номера с уверенностью; целевая метрика
качества — Full Sequence Accuracy > 85% на валидационной выборке (CCPD / Nomeroff Net).
