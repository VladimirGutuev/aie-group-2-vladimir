...
> **Чем проект отличается.** Современная, лёгкая и самодостаточная альтернатива тяжёлым/закрытым
> ANPR-решениям, с точностью на уровне production:
> - **современный стек** (torch 2.5, YOLOv8, FastAPI) — не legacy;
> - **своя CRNN 99.54%** ≈ production-Nomeroff 99.58% (1 номер из 2845);
> - **edge-вариант CRNN-tiny** — в 1.8× меньше и быстрее Nomeroff при 98.66%;
> - **2 рантайм-зависимости** (`torch`+`opencv`) против тяжёлого фреймворка с конфликтами версий.
...
- **Краткое описание:**
  Проект — это REST API-сервис, который по изображению с камеры возвращает распознанный номерной знак автомобиля и уверенность модели. Решение строится как двухступенчатый пайплайн: детекция области номера (обученный YOLOv8n, test mAP@50 0.99) и OCR (собственная обученная CRNN, test FSA 99.54%); EasyOCR используется как baseline для сравнения. Сервис задумывается как универсальный backend для разных сценариев СКУД: умный шлагбаум, автоматическая оплата парковки в ТЦ, журнал въездов/выездов на охраняемых объектах. Метрика успеха — Full Sequence Accuracy > 85% — достигнута с запасом.
