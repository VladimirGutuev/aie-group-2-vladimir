...
| 2  | Endpoint `/predict` использует **реальную модель**, а не заглушку        | ✅               | `/predict` использует EasyOCR (`src/models/ocr.py`); YOLOv8 включается флагом `USE_DETECTOR` |
| 3  | Есть EDA и хотя бы один эксперимент с метриками                          | ❌               | TODO: `notebooks/01_eda.ipynb`                      |
| 4  | Есть baseline и улучшенная модель, есть **сравнение по метрикам**        | ❌               | TODO: baseline = только OCR; улучшенная = YOLO+OCR  |
| 5  | Код не свален в один ноутбук: есть внятная структура в `src/`            | ✅               | `src/service/`, `src/models/`, `src/pipeline.py`    |
...
| 9  | В `report.md` **обоснован выбор финальной модели** по результатам экспериментов | ❌        | TODO: после прогона экспериментов                    |
