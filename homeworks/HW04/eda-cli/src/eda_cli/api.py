...
class QualityFlagsResponse(BaseModel):
    """Ответ с полным набором флагов качества датасета (HW03)."""
...
async def quality_flags_from_csv(
    ...
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
    (summarize_dataset + missing_table + compute_quality_flags)
    и возвращает полный набор флагов качества в json формате.
    
    Флаги включают базовые эвристики и новые из HW03:
    ...
    - has_high_cardinality_categoricals: категориальные колонки с > 50 уникальными значениями
    - has_many_zero_values: числовые колонки с долей нулей > 50%
    """
    ...
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    # Передаём df для корректной работы эвристики has_many_zero_values
    flags_all = compute_quality_flags(summary, missing_df, df=df)

    # Оставляем только булевые флаги согласно требованиям задания
    flags_bool: dict[str, bool] = {
        key: bool(value)
        for key, value in flags_all.items()
        if isinstance(value, bool)
